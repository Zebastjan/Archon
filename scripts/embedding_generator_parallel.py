...
async def process_entity(
    entity: dict, db, client: httpx.AsyncClient, sem: asyncio.Semaphore
) -> bool:
...
async def process_batch_parallel(
    entities: list, db, client: httpx.AsyncClient, max_concurrent: int
) -> tuple[int, int]:
...
async def main():
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        while True:
            ...
            succeeded, failed = await process_batch_parallel(
                entities, db, client, MAX_CONCURRENT
            )
            ...
            print(
                f"  ✓ {succeeded} ✓, {failed} ✗ in {batch_time:.1f}s | "
