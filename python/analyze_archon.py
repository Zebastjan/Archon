...
from src.server.services.languages.language_support import CodeEntity, CodeRelationship
...
class CodebaseAnalyzer:
    ...
    def _parse_file(self, file_path: Path, lang_support):
        ...
        try:
            ...
            # Tag entities with file info
            for e in entities:
                if not e.metadata:
                    e.metadata = {}
                e.metadata['file_path'] = str(rel_path)
                e.metadata['language'] = lang_support.language_id
    ...
    def _analyze_complexity(self) -> dict[str, Any]:
        ...
        if not sizes:
            return {
                "size_distribution": {"min_lines": 0, "max_lines": 0, "avg_lines": 0, "median_lines": 0},
                "large_entities_count": 0,
                "largest_entities": [],
            }
    ...
    def _analyze_documentation(self) -> dict[str, Any]:
        ...
        type_coverage = {
            t: {
                ...
                "coverage_pct": round(data["with_docs"] / data["total"] * 100, 1) if data["total"] > 0 else 0
            }
    ...
    def _analyze_modules(self) -> dict[str, Any]:
        ...
        return {
            ...
            "module_distribution": dict(sorted(module_groups.items(), key=lambda x: x[1], reverse=True)),
        }
    
    def _analyze_relationships(self) -> dict[str, Any]:
        """Analyze entity relationships."""
        if not self.relationships:
            return {"total_relationships": 0, "by_type": {}, "most_connected_entities": []}
...
def print_report(report: dict[str, Any]):
    ...
    print("\n📁 MODULE STRUCTURE")
    print("-" * 40)
    modules = report["module_structure"]
    print(f"  Files with entities: {modules['total_files_with_entities']}")
    print(f"  Avg entities/file:   {modules['avg_entities_per_file']}")
    print("\n  Top modules:")
    for mod, count in list(modules['module_distribution'].items())[:5]:
    ...
    for layer, count in sorted(arch["detected_layers"].items(), key=lambda x: x[1], reverse=True):
...
def main():
    """Main entry point."""
    # Analyze the python/src directory
    ...
    # Save JSON report
    report_path = Path(__file__).parent / "codebase_analysis_report.json"
