...
class GitRepositoryManager:
    ...
    async def register_local_repo(
        ...
        try:
            result = subprocess.run(
                ["git", "-C", str(local_path), "rev-parse", "HEAD"], capture_output=True, text=True, check=True
            )
    ...
    async def _create_repo_in_db(
        ...
        existing = await db.fetch("SELECT id FROM archon_code_repos WHERE local_path = $1", local_path)
...
'''
    ...
    async def incremental_update(self, repo_id: str, changed_files: list[str]):
        ...
        supported_files = [
            f for f in changed_files if f.endswith((".py", ".ts", ".tsx", ".js", ".jsx", ".nim", ".nims"))
        ]
        ...
        try:
            result = subprocess.run(
                ["git", "-C", str(config.local_path), "rev-parse", "HEAD"], capture_output=True, text=True, check=True
            )
    ...
    async def full_reingest(self, repo_id: str):
        ...
        db = await self._ensure_db()
        await db.execute("DELETE FROM archon_code_entities WHERE repo_id = $1", repo_id)
        ...
        file_paths = [str(f.relative_to(config.local_path)) for f in all_files]
    ...
    async def get_file_url(self, repo_id: str, file_path: str, line: int | None = None) -> str | None:
        ...
        url = (
            f"https://github.com/{github_info['owner']}/{github_info['repo']}/blob/{github_info['branch']}/{file_path}"
        )
