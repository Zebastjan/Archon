...
class LanguageSupportRegistry:
    ...
    def register(self, support: LanguageSupport) -> None:
        ...
        self._logger.info(f"Language registered: {support.language_id} with extensions {support.file_extensions}")
...
def _register_builtin_languages(registry: LanguageSupportRegistry) -> None:
    ...
    try:
        ...
        logger.warning(f"Nim language support not available: {e}")

    try:
        from .tla_support import TlaLanguageSupport

        registry.register(TlaLanguageSupport())
        logger.debug("TLA+ language support registered")
    except (ImportError, Exception) as e:
        logger.warning(f"TLA+ language support not available: {e}")

    logger.info(
        f"Builtin languages registered: {len(registry.list_languages())} ({', '.join(registry.list_languages())})"
    )
