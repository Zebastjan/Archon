...
class CodebaseMCPTools:
    ...
    async def codebase_find_entity(self, repo_id: str, query: str, entity_type: str | None = None) -> dict[str, Any]:
        ...
        Args:
            repo_id: Repository UUID (use 'archon-python' or 'archon-ui' for known repos)
    ...
    async def codebase_semantic_search(self, repo_id: str, query: str, limit: int = 10) -> dict[str, Any]:
        ...
        Example:
            ...
            >>> await codebase_semantic_search("archon-ui", "React component for modals")
        """
        try:
    ...
    async def codebase_get_entity_context(self, entity_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Get relationships
            outgoing = await self._code_service.get_entity_relationships(
            ...
            return {
                ...
                "relationships": {
                    "calls": [  # This entity calls these
                        ...
                        for r in outgoing if r["relationship_type"] == "CALLS"
                    ],
                    "called_by": [  # These call this entity
                        {"name": r["entity_name"], "type": r["entity_type"], "file": r["file_path"]}
                        for r in incoming if r["relationship_type"] == "CALLS"
                    ],
                    "inherits_from": [
                        {"name": r["entity_name"], "type": r["entity_type"]}
                        for r in outgoing if r["relationship_type"] == "INHERITS"
                    ],
                    "inherited_by": [
                        {"name": r["entity_name"], "type": r["entity_type"]}
                        for r in incoming if r["relationship_type"] == "INHERITS"
                    ],
                }
            }
        except Exception as e:
