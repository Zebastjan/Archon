...
def init_postgres():
    ...
    result = subprocess.run(
        ["pg_ctl", "-D", str(data_dir), "initdb"], capture_output=True, text=True
    )
...
def start_postgres():
    ...
    result = subprocess.run(
        ["pg_ctl", "-D", str(data_dir), "status"], capture_output=True, text=True
    )
    ...
    for _ in range(30):
        time.sleep(0.5)
        result = subprocess.run(
            ["pg_isready", "-h", "127.0.0.1", "-p", "5433"], capture_output=True
        )
        if result.returncode == 0:
...
def create_database():
    ...
    result = subprocess.run(
        [
            ...
            "postgres",
            "-c",
            "SELECT 1 FROM pg_database WHERE datname='archon'",
        ],
        capture_output=True,
        text=True,
    )
